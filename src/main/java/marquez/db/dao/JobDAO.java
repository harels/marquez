...
public interface JobDAO extends SqlObject {
  ...
  default void updateOwnership(final String ownerName, final String jobName) {
    try (final Handle handle = getHandle()) {
      handle.useTransaction(
          h -> {
            ...
            int jobId =
                h.createQuery("SELECT id FROM jobs WHERE name = :jobName")
                    ...
                    .mapTo(int.class)
                    .findOnly();
            int ownershipId = ownershipDAO.insert(jobName, ownerName);
            h.createUpdate("UPDATE jobs SET current_ownership = :ownershipId, updated_at = NOW() WHERE id = :jobId")
                .bind("ownershipId", ownershipId)
                .bind("jobId", jobId)
                .execute();
          });
